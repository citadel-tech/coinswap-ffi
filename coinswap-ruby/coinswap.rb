...
def self.uniffi_bytes(v)
  ...
  # The Record type AddressType.

  def self.check_lower_TypeAddressType(v)
    
  end

  def self.alloc_from_TypeAddressType(v)
    RustBuffer.allocWithBuilder do |builder|
      builder.write_TypeAddressType(v)
      return builder.finalize
    end
  end

  def consumeIntoTypeAddressType
    consumeWithStream do |stream|
      return stream.readTypeAddressType
    end
  end
  ...
  # The Record type MakerOfferCandidate.

  def self.check_lower_TypeMakerOfferCandidate(v)
    RustBuffer.check_lower_TypeMakerAddress(v.address)
    RustBuffer.check_lower_OptionalTypeOffer(v.offer)
    RustBuffer.check_lower_TypeMakerState(v.state)
    RustBuffer.check_lower_OptionalTypeMakerProtocol(v.protocol)
  end

  def self.alloc_from_TypeMakerOfferCandidate(v)
    RustBuffer.allocWithBuilder do |builder|
      builder.write_TypeMakerOfferCandidate(v)
      return builder.finalize
    end
  end

  def consumeIntoTypeMakerOfferCandidate
    consumeWithStream do |stream|
      return stream.readTypeMakerOfferCandidate
    end
  end

  # The Record type MakerProtocol.

  def self.check_lower_TypeMakerProtocol(v)
    
  end

  def self.alloc_from_TypeMakerProtocol(v)
    RustBuffer.allocWithBuilder do |builder|
      builder.write_TypeMakerProtocol(v)
      return builder.finalize
    end
  end

  def consumeIntoTypeMakerProtocol
    consumeWithStream do |stream|
      return stream.readTypeMakerProtocol
    end
  end

  # The Record type MakerState.

  def self.check_lower_TypeMakerState(v)
    
    RustBuffer.check_lower_Optionalu8(v.retries)
  end

  def self.alloc_from_TypeMakerState(v)
    RustBuffer.allocWithBuilder do |builder|
      builder.write_TypeMakerState(v)
      return builder.finalize
    end
  end

  def consumeIntoTypeMakerState
    consumeWithStream do |stream|
      return stream.readTypeMakerState
    end
  end
  ...
  def self.check_lower_TypeOfferBook(v)
    RustBuffer.check_lower_SequenceTypeMakerOfferCandidate(v.makers)
  end
  ...
  def self.check_lower_Optionalu8(v)
    if not v.nil?
      
    end
  end

  def self.alloc_from_Optionalu8(v)
    RustBuffer.allocWithBuilder do |builder|
      builder.write_Optionalu8(v)
      return builder.finalize()
    end
  end

  def consumeIntoOptionalu8
    consumeWithStream do |stream|
      return stream.readOptionalu8
    end
  end
  ...
  # The Optional<T> type for TypeMakerProtocol.

  def self.check_lower_OptionalTypeMakerProtocol(v)
    if not v.nil?
      RustBuffer.check_lower_TypeMakerProtocol(v)
    end
  end

  def self.alloc_from_OptionalTypeMakerProtocol(v)
    RustBuffer.allocWithBuilder do |builder|
      builder.write_OptionalTypeMakerProtocol(v)
      return builder.finalize()
    end
  end

  def consumeIntoOptionalTypeMakerProtocol
    consumeWithStream do |stream|
      return stream.readOptionalTypeMakerProtocol
    end
  end

  # The Optional<T> type for TypeOffer.

  def self.check_lower_OptionalTypeOffer(v)
    if not v.nil?
      RustBuffer.check_lower_TypeOffer(v)
    end
  end

  def self.alloc_from_OptionalTypeOffer(v)
    RustBuffer.allocWithBuilder do |builder|
      builder.write_OptionalTypeOffer(v)
      return builder.finalize()
    end
  end

  def consumeIntoOptionalTypeOffer
    consumeWithStream do |stream|
      return stream.readOptionalTypeOffer
    end
  end
  ...
  # The Sequence<T> type for TypeMakerOfferCandidate.

  def self.check_lower_SequenceTypeMakerOfferCandidate(v)
    v.each do |item|
      RustBuffer.check_lower_TypeMakerOfferCandidate(item)
    end
  end

  def self.alloc_from_SequenceTypeMakerOfferCandidate(v)
    RustBuffer.allocWithBuilder do |builder|
      builder.write_SequenceTypeMakerOfferCandidate(v)
      return builder.finalize()
    end
  end

  def consumeIntoSequenceTypeMakerOfferCandidate
    consumeWithStream do |stream|
      return stream.readSequenceTypeMakerOfferCandidate
    end
  end
...
class RustBufferStream
  ...
  def readTypeAddressType
    AddressType.new(
      addr_type: readString
    )
  end
  ...
  def readTypeMakerOfferCandidate
    MakerOfferCandidate.new(
      address: readTypeMakerAddress,
      offer: readOptionalTypeOffer,
      state: readTypeMakerState,
      protocol: readOptionalTypeMakerProtocol
    )
  end

  # The Record type MakerProtocol.

  def readTypeMakerProtocol
    MakerProtocol.new(
      protocol_type: readString
    )
  end

  # The Record type MakerState.

  def readTypeMakerState
    MakerState.new(
      state_type: readString,
      retries: readOptionalu8
    )
  end
  ...
  def readTypeOfferBook
    OfferBook.new(
      makers: readSequenceTypeMakerOfferCandidate
    )
  end
  ...
  def readOptionalTypeMakerProtocol
    flag = unpack_from 1, 'c'

    if flag == 0
      return nil
    elsif flag == 1
      return readTypeMakerProtocol
    else
      raise InternalError, 'Unexpected flag byte for OptionalTypeMakerProtocol'
    end
  end

  # The Optional<T> type for TypeOffer.

  def readOptionalTypeOffer
    flag = unpack_from 1, 'c'

    if flag == 0
      return nil
    elsif flag == 1
      return readTypeOffer
    else
      raise InternalError, 'Unexpected flag byte for OptionalTypeOffer'
    end
  end
  ...
  def readSequenceTypeMakerOfferCandidate
    count = unpack_from 4, 'l>'
    ...
    count.times do
      items.append readTypeMakerOfferCandidate
    end
...
class RustBufferBuilder
  ...
  def write_U8(v)
    v = Coinswap::uniffi_in_range(v, "u8", 0, 2**8)
    pack_into(1, 'c', v)
  end
  ...
  def write_TypeAddressType(v)
    self.write_String(v.addr_type)
  end
  ...
  def write_TypeMakerOfferCandidate(v)
    self.write_TypeMakerAddress(v.address)
    self.write_OptionalTypeOffer(v.offer)
    self.write_TypeMakerState(v.state)
    self.write_OptionalTypeMakerProtocol(v.protocol)
  end

  # The Record type MakerProtocol.

  def write_TypeMakerProtocol(v)
    self.write_String(v.protocol_type)
  end

  # The Record type MakerState.

  def write_TypeMakerState(v)
    self.write_String(v.state_type)
    self.write_Optionalu8(v.retries)
  end
  ...
  def write_TypeOfferBook(v)
    self.write_SequenceTypeMakerOfferCandidate(v.makers)
  end
  ...
  def write_OptionalTypeMakerProtocol(v)
    if v.nil?
      pack_into(1, 'c', 0)
    else
      pack_into(1, 'c', 1)
      self.write_TypeMakerProtocol(v)
    end
  end

  # The Optional<T> type for TypeOffer.

  def write_OptionalTypeOffer(v)
    if v.nil?
      pack_into(1, 'c', 0)
    else
      pack_into(1, 'c', 1)
      self.write_TypeOffer(v)
    end
  end
  ...
  def write_SequenceTypeMakerOfferCandidate(items)
    pack_into(4, 'l>', items.size)

    items.each do |item|
      self.write_TypeMakerOfferCandidate(item)
    end
  end
...
module UniFFILib
  ...
  attach_function :uniffi_coinswap_ffi_checksum_method_taker_get_wallet_name,
    [RustCallStatus.by_ref],
    :uint16
  attach_function :uniffi_coinswap_ffi_checksum_method_taker_is_offerbook_syncing,
    [RustCallStatus.by_ref],
    :uint16
  ...
  attach_function :uniffi_coinswap_ffi_checksum_method_taproottaker_get_wallet_name,
    [RustCallStatus.by_ref],
    :uint16
  attach_function :uniffi_coinswap_ffi_checksum_method_taproottaker_is_offerbook_syncing,
    [RustCallStatus.by_ref],
    :uint16
...
class AddressType
  attr_reader :addr_type

  def initialize(addr_type:)
    @addr_type = addr_type
  end

  def ==(other)
    if @addr_type != other.addr_type
      return false
    end
...
class MakerFeeInfo
  ...
  # Record type MakerOfferCandidate
class MakerOfferCandidate
  attr_reader :address, :offer, :state, :protocol

  def initialize(address:, offer:, state:, protocol:)
    @address = address
    @offer = offer
    @state = state
    @protocol = protocol
  ...
  # Record type MakerProtocol
class MakerProtocol
  attr_reader :protocol_type

  def initialize(protocol_type:)
  ...
  # Record type MakerState
class MakerState
  attr_reader :state_type, :retries

  def initialize(state_type:, retries:)
...
class UtxoSpendInfo
  attr_reader :spend_type, :path, :multisig_redeemscript, :input_value, :index

  def initialize(spend_type:, path:, multisig_redeemscript:, input_value:, index:)
