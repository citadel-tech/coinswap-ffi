...
private fun uniffiCheckApiChecksums(lib: IntegrityCheckingUniffiLib) {
    ...
    if (lib.uniffi_coinswap_ffi_checksum_method_taker_get_all_good_makers() != 59201.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    if (lib.uniffi_coinswap_ffi_checksum_method_taker_get_balances() != 61330.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    if (lib.uniffi_coinswap_ffi_checksum_method_taker_get_next_external_address() != 20757.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    if (lib.uniffi_coinswap_ffi_checksum_method_taker_get_next_internal_addresses() != 36525.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    ...
    if (lib.uniffi_coinswap_ffi_checksum_method_taker_send_to_address() != 59663.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
...
public object FfiConverterSequenceTypeOfferAndAddress: FfiConverterRustBuffer<List<OfferAndAddress>> {
    override fun read(buf: ByteBuffer): List<OfferAndAddress> {
        ...
        return List<OfferAndAddress>(len) {
            FfiConverterTypeOfferAndAddress.read(buf)
        }
    }

    override fun allocationSize(value: List<OfferAndAddress>): ULong {
        val sizeForLength = 4UL
        val sizeForItems = value.map { FfiConverterTypeOfferAndAddress.allocationSize(it) }.sum()
        return sizeForLength + sizeForItems
    }
