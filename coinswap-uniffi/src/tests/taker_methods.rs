...
fn cleanup_wallet() {
    ...
    // Clean up watcher registry to force fresh discovery
    let mut watcher_dir = PathBuf::from(env!("HOME"));
    watcher_dir.push(".coinswap");
    watcher_dir.push("taker");
    watcher_dir.push(".taker_watcher");

    if watcher_dir.exists() {
        let _ = fs::remove_dir_all(&watcher_dir);
        println!("✓ Cleaned up watcher registry (will force fresh discovery)");
    }
    ...
    let output = Command::new("docker")
        .args(&["exec", "coinswap-ffi-bitcoind", "rm", "-rf", "/home/bitcoin/.bitcoin/wallets/test-taker"])
        .output();
...
fn test_taker_complete_flow() {
    ...
    let external_address1 = taker.get_next_external_address();
    assert!(
        external_address1.is_ok(),
        "Should generate external address successfully"
    );

    let external_address2 = taker.get_next_external_address();
    assert!(
    ...
    let _txid = bitcoind.send_to_address_from_funding_wallet(&funding_address, fund_amount).unwrap();
    taker.sync_and_save().unwrap();
    ...
    let good_makers = taker.get_all_good_makers();
    println!("Good makers: {:?}", good_makers);

    let fetch_good_makers_result = taker.fetch_good_makers();
    println!("Fetch good makers result: {:?}", fetch_good_makers_result);

    let fetch_offers_result = taker.fetch_offers();
    println!("Fetch offers result: {:?}", fetch_offers_result);
    
    // If no makers found, wait a bit more and retry
    if let Ok(offers) = &fetch_offers_result {
        if offers.good_makers.is_empty() {
            println!("\nNo makers found yet, waiting 20 more seconds...");
            std::thread::sleep(std::time::Duration::from_secs(20));
            let fetch_offers_result = taker.fetch_offers();
            println!("Retry fetch offers result: {:?}", fetch_offers_result);
        }
    }
